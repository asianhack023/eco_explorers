...
const LoginSignup = () => {
  ...
  const login = async () => {
    ...
    try {
      const response = await fetch("https://fb41-202-51-68-213.ngrok-free.app/login", {
        method: "POST",
        headers: {
  ...
  const signup = async () => {
    ...
    try {
      const response = await fetch("https://fb41-202-51-68-213.ngrok-free.app/register", {
        method: "POST",
        body: formDataToSend,
      });
      ...
      if (responseData) {
        ...
        console.error("SignUp Failed - Error Message:", responseData.errors);
  ...
  return (
    <div className="w-full h-auto bg-pink-200 pt-14 pb-8 mb-[25vw]">
      <div className="w-11/12 max-w-lg h-auto bg-white mx-auto p-5">
        ...
        <button
          ...
          className="w-full h-14 text-white bg-red-500 mt-5 border-none text-lg font-medium cursor-pointer"
        >
          Continue
        </button>
        {state === "Sign Up" ? (
